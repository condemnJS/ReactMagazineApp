...
export const Head = () => {
    ...
    const getCategories = useCallback(async () => {
        try {
            const data = await request(
                'http://magazine.test/api/categories'
            )
    ...
    const spanAnimateBurger = () => {
        // Animation Burger
        const burgerItem = document.querySelectorAll('.elementBurger');
        burgerItem[0].classList.toggle('toggleClassSpanAnimatePlus');
        burgerItem[1].classList.toggle('d-none');
        burgerItem[2].classList.toggle('toggleClassSpanAnimateMinus');

        // Layout wrapper control
        const layoutWrapper = document.querySelector('.layout-wrapper');
        layoutWrapper.classList.toggle('animateLayoutWrapper');

        // Layout modal
        const layoutModal = document.querySelector('.layout-modal');
        layoutModal.classList.toggle('animateLayout')

        const sidebar = document.querySelector('.layout__sidebar');
        sidebar.classList.toggle('animateSidebar')
    }
    return (
        <Fragment>
            <div className={'layout-wrapper animateLayoutWrapper'}></div>
            <div className={'head-wrapper d-f ju-c'}>
                <div className="header-wrapper container d-f ju-c w-100">
                    <div className="head__logo d-f al-c">
                        <span className={'d-f'}><b className={'mr-1'}>Татар</b> Маркет</span>
                    </div>
                    <div className="head__burger d-f al-c">
                        <div className="burger__item catalog-menu d-f ju-c al-c" onClick={spanAnimateBurger}>
                            <div>
                        ...
                        <div className="burger__item catalog-title d-f al-c" onClick={spanAnimateBurger}>
                ...
                <div className="layout-modal">
                    <div className="container">
                        <div className="layout__sidebar d-f fl-c">
                            {categories.map(item => {
                                return (
                                    <a href={item.slug} key={item.id} className={'d-f al-c'}>
                                        <img src={'../../img/icons/categories/' + item.icon} alt=""/>
